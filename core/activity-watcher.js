...
const SERVER_PORT = Number(process.env.SERVER_PORT || 65000);
const SERVER_PATH = process.env.SERVER_PATH || "/app-watcher";
...
function tryXpropWmClass(windowId) {
  try {
    ...
    const out = execSync(`xprop -id ${winHex} WM_CLASS`, {
      encoding: "utf8",
      timeout: 1000,
    }).trim();
...
function normalizeAppHeuristic(rawOwnerName, ownerPath, title, windowId) {
  if (rawOwnerName) {
    ...
    if (key.length > 1)
      return {
        app: titleCase(key.replace(/[-_\.]/g, " ")),
        reason: "owner-titlecase",
      };
  }
  ...
  if (ownerPath) {
    ...
    if (k) {
      if (k.includes("gnome-control-center") || k.includes("control-center"))
        return { app: "Settings", reason: "ownerpath" };
      if (/gnome-terminal|tilix|konsole|alacritty|kitty|xterm/.test(k))
        return { app: "Terminal", reason: "ownerpath" };
      ...
      if (k.includes("nautilus") || k.includes("nemo"))
        return { app: "Files", reason: "ownerpath" };
      return {
        app: titleCase(k.replace(/[-_\.]/g, " ")),
        reason: "ownerpath-fallback",
      };
    }
  }
  ...
  if (wmclass) {
    ...
    if (key.includes("gnome-control-center"))
      return { app: "Settings", reason: "xprop-wmclass" };
    if (/terminal|konsole|alacritty|kitty/.test(key))
      return { app: "Terminal", reason: "xprop-wmclass" };
    if (key.includes("code"))
      return { app: "VS Code", reason: "xprop-wmclass" };
    if (key.includes("brave")) return { app: "Brave", reason: "xprop-wmclass" };
    if (key.includes("chrome"))
      return { app: "Chrome", reason: "xprop-wmclass" };
    if (key.includes("firefox"))
      return { app: "Firefox", reason: "xprop-wmclass" };
    return {
      app: titleCase(key.replace(/[-_\.]/g, " ")),
      reason: "xprop-wmclass",
    };
  }
  ...
  if (title) {
    ...
    if (
      t.includes("settings") ||
      t.includes("control center") ||
      t.includes("gnome control")
    )
      return { app: "Settings", reason: "title-contains" };
    if (
      t.includes("terminal") ||
      t.includes("bash") ||
      t.includes("zsh") ||
      ...
      return { app: "Terminal", reason: "title-contains" };
    if (t.includes("file") || t.includes("nautilus") || t.includes("files"))
      return { app: "Files", reason: "title-contains" };
    if (t.includes("vscode") || t.includes("visual studio code"))
      return { app: "VS Code", reason: "title-contains" };
    if (t.includes("firefox") || t.includes("mozilla"))
      return { app: "Firefox", reason: "title-contains" };
    if (t.includes("chrome") || t.includes("brave"))
      return { app: "Browser", reason: "title-contains" };
  }

  return {
    app: titleCase(title || rawOwnerName || "unknown"),
    reason: "fallback",
  };
}
...
async function mainLoop() {
  console.log(
    "[activewin-watcher] starting; POST ->",
  ...
  while (true) {
    try {
      ...
      const ownerName =
        (raw.owner && (raw.owner.name || raw.owner.processName)) || null;
      ...
      if (lastSeen.app === appNorm && lastSeen.title === title) {
        if (
          now - lastSeen.at >= DEBOUNCE_MS &&
          now - lastSentAt >= DEBOUNCE_MS
        ) {
          const payload = {
            event: "appSwitch",
            timestamp: now,
            prev: lastSeen.app
              ? { app: lastSeen.app, title: lastSeen.title }
              : null,
            next: {
              id: `${appNorm}-${now}`,
              app: appNorm,
              title: title || null,
              pid: raw.owner?.processId ?? null,
            },
            context: {
              reason: "active-win-improved",
              confidence: 1.0,
              heuristic: norm.reason,
            },
            source: {
              watcherId: "activewin-watcher-improved",
              hostId: os.hostname(),
            },
          };
          ...
          try {
            ...
            console.log(
              "[activewin-watcher] posted ->",
              payload.next.app,
              payload.next.title ? `(${payload.next.title})` : "",
              "via",
              norm.reason,
            );
            lastSentAt = now;
